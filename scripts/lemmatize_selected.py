...
df = pd.read_csv('data/processed/symptom2disease_clean.csv', encoding='utf-8-sig')
print(f"   Загружено {len(df)} строк")

tqdm.pandas(desc="Лемматизация Symptom2Disease")
df['text_lemmatized'] = df['text'].progress_apply(lemmatize_text)

df_lemmatized = df[['text_lemmatized', 'label']].rename(columns={'text_lemmatized': 'text'})
df_lemmatized.to_csv('data/processed/symptom2disease_lemmatized.csv', index=False, encoding='utf-8-sig')
...
df_lemmatized = df[['text_lemmatized', 'label']].rename(columns={'text_lemmatized': 'text'})
df_lemmatized.to_csv('data/processed/diagnozes_lemmatized.csv', index=False, encoding='utf-8-sig')
print(f"   Сохранено {len(df_lemmatized)} строк в data/processed/diagnozes_lemmatized.csv")
print(f"   Пример:\n     Было: {df['text'].iloc[0][:80]}...")
print(f"     Стало: {df_lemmatized['text'].iloc[0][:80]}...")
